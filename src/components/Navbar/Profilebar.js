    ...
    <div>
      <Menu as="div" class="  relative inline-block text-left mr-2 mt-1">
        ...
        >
              ...
              </span>
              {
                !user && (
                  <Menu.Item>
                    {({close}) => (
                      <div>
                        <button class="text-pink-600" onClick={() => {openSignatureModal(); close()}}>Sign in</button> to see your level
                        status
                      </div>
                    )}
                  </Menu.Item>
          ...
          <hr class="bg-gray-300 opacity-100 mx-2 h-px"></hr>
          <Link class="flex items-center space-x-4 p-4 font-semibold   w-full hover:bg-app" to='profile'>
            <RiSettingsFill />
          ...
          <Menu.Item>
            {({close}) => (
              <div onClick={() => {!user ? openSignatureModal() : signOut(); close()}} className="flex items-center space-x-4 p-4 font-semibold w-full hover:bg-app cursor-pointer">
                <BsPersonFill />
