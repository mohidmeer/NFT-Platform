...
const Intro = ({stateChanger, setValues, values}) => {
  ...
  return (
    <div className='w-1/3'>
      ...
      <RadioGroup value={values.blockchain} onChange={(e) => setValues({...values, blockchain: e})}>
        <RadioGroup.Label></RadioGroup.Label>
        <RadioGroup.Option value="ethereum">
          {({checked}) => (
            <span className={` ${checked ? 'dsa' : 'das'} flex justify-between font-bold p-4 bg-white rounded-lg w-full mt-2`}>Ethereum {checked ? <FiCheck className='text-pink-600 text-3xl bg-app rounded-full p-1' /> : ''}</span>
          )}
        </RadioGroup.Option>
      ...
      <RadioGroup value={values.copyright} onChange={(e) => setValues({...values, copyright: e})}>
        <RadioGroup.Label></RadioGroup.Label>
      ...
      {location.pathname === '/createwhitelist' ? <WhitelistRequirements /> : ''}
