          ...
          {plans.map((plan, i) => {
            return (
              <div className="bg-white p-6 shadow-xl  rounded-xl border border-green-400">
                ...
                <button
                  class="flex justify-center text-white font-extrabold bg-pink-600 hover:bg-green-700 focus:ring-4 focus:ring-green-200  rounded-lg text-xl px-5 py-2.5 text-center dark:text-white dark:focus:ring-pink-900 mx-auto"
                  onClick={() => signUpUser(plan.name.toUpperCase())}
                >
                  {plan.name + " Mint"}
                </button>
              </div>
